...
### Calculate EflowStats - POF Rates
# The following function runs the new EflowStats package and calculates Stream Flow Characterists for each POF rate withdrawal scenario
# 
...
# (3) subtracts POF to generate withdrawal scenario and SETS MINIMUM FLOW VALUE XX? cfs
# (4) calculates SFCs for each POF withdrawal scenario
# 
# Notes: 
...
getFlowStatsPOF <- function(siteID, startDtNew, endDtNew, mflNorm = NULL, maxCap = NULL) {
	dvs <- tryCatch({
			...
			if(nrow(pkFile) < 2) {
				...
				for(i in seq(1, length(perSeq), 1)) {
					...
					if (is.null(mflNorm) == TRUE) {
						...
						mfl <- drainageArea * mflNorm
						
					}
				  
				  # maximum pumping capacity of 50cfs (~32 MGD)
				  if (is.null(maxCap) == TRUE) {
					...
					for (j in 1:nrow(newDvs)) {
						...
						# subtraction value base on percentage of unaltered flow value
						subVal <- dvs[j,2] * perSeq[i]

						# Set max pumping capacity to 50cfs
						subVal <- if_else(subVal <= 50, subVal, 50)
						
						if(dvs[j,2] - subVal >= mfl) {
							
							newDvs[j,2] <- dvs[j,2] - subVal
							
						} 
						
						else if(dvs[j,2] >= mfl & dvs[j,2] - subVal < mfl) {
...
# test <- getFlowStatsPOF(siteID = siteDates[213,"site_no"],
# 													startDtNew = siteDates[213,"startDateNew"],
# 													endDtNew = siteDates[213,"endDateNew"])
# 
# for (i in 214:216) {
# 	test2 <- getFlowStatsPOF(siteID = siteDates[i,"site_no"],
# 													startDtNew = siteDates[i,"startDateNew"],
# 													endDtNew = siteDates[i,"endDateNew"])
# 
# 	test <- rbind(test, test2); rm(test2)
# }
# 
# 
# dvs <- readNWISdv(siteDates[215,"site_no"], parameterCd = "00060", startDate = siteDates[215,"startDateNew"], endDate = siteDates[215,"endDateNew"], statCd = "00003")
# dvs <- dvs[,c(3,4)]
# colnames(dvs) <- c("Date", "Flow")
# dailyQClean <- validate_data(dvs[c("Date", "Flow")], yearType = "water")
# 
# gaps <- getDateGaps(siteDates[215,"site_no"])
